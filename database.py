...
def abrir_planilha(nome, max_tentativas=5):
    """Abre a worksheet com retry e backoff exponencial para erros transitórios."""
    client = conectar()
    for tentativa in range(max_tentativas):
        try:
            ...
            if codigo in (429, 500, 503) and tentativa < max_tentativas - 1:
...
def _executar_com_retry(func, max_tentativas=5):
    """
    Executa func() com retry e backoff exponencial com jitter.
    Trata erros 429 (quota), 500 (erro interno) e 503 (indisponível) do Google Sheets API.
    """
    for tentativa in range(max_tentativas):
        try:
            return func()
            ...
            codigo = getattr(getattr(e, "response", None), "status_code", 0)
            if codigo in (429, 500, 503) and tentativa < max_tentativas - 1:
                espera = (2 ** tentativa) + random.uniform(0, 1)  # backoff + jitter
                time.sleep(espera)
                continue
            raise
        except Exception:
...
def salvar_implantacao(df):
    ...
    df = df.astype(str)
    _executar_com_retry(lambda: ws.clear())
    _executar_com_retry(lambda: ws.append_row(SCHEMAS["implantacao"]))
    if not df.empty:
        linhas = [[row[col] for col in SCHEMAS["implantacao"]] for _, row in df.iterrows()]
        _executar_com_retry(lambda: ws.append_rows(linhas, value_input_option="RAW"))
    limpar_cache()
