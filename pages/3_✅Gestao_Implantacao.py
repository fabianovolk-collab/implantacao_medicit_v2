...
def inicializar_formulario(cnpj, etapa, v):
    ...
    Isso evita sobrescrever o que o usuário está preenchendo,
    mas garante isolamento total ao trocar de cliente ou etapa.
    """
    ctx = chave_contexto(cnpj, etapa)
    if st.session_state.get("impl_contexto_ativo") == ctx:
        return  # mesmo contexto → não reinicializa → usuário pode editar livremente
...
if not registro_existente.empty:
    ...
    v.update({
        ...
        "resp_medicit":  reg.get("responsavel_medicit", ""),
        "resp_cliente":  reg.get("responsavel_cliente", ""),
...
inicializar_formulario(chave_cliente, etapa_selecionada, v)
...
motivo = st.text_input("Motivo / Observação", key=k("motivo"))
