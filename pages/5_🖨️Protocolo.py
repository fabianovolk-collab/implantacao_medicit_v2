...
def gerar_pdf(df_cliente, cliente, progresso, doc_sel):
    ...
    for _, row in df_cliente.iterrows():
        ...
        pdf.cell(190, 6, f"Etapa: {limpar_texto(etapa_nome)}", 0, 1)
        pdf.set_font("Arial", "", 10)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Status: {limpar_texto(row.get('Status',''))}", 0, 1)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Responsavel Medicit: {limpar_texto(row.get('Responsavel_Etapa',''))}", 0, 1)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Responsavel Cliente: {limpar_texto(row.get('Responsavel_Cliente',''))}", 0, 1)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Participantes: {limpar_texto(row.get('Participantes',''))}", 0, 1)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Data Inicio: {formatar_data(row.get('Data_Inicio',''))}", 0, 1)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Data Conclusao: {formatar_data(row.get('Data_Conclusao',''))}", 0, 1)
        ...
        if base:
            # Garante que X volta à margem esquerda após qualquer multi_cell anterior
            pdf.set_x(10)
            pdf.set_font("Arial", "B", 10)
            pdf.cell(190, 5, "Checklist:", 0, 1)
            pdf.set_font("Arial", "", 10)
            for item in base:
                marcacao = "X" if item in salvo else " "
                # set_x(10) antes de cada multi_cell evita deslocamento lateral
                pdf.set_x(10)
        ...
        pdf.ln(2)
        pdf.set_x(10)
        pdf.cell(190, 5, f"Progresso Etapa: {prog}% - {status_texto}", 0, 1)
