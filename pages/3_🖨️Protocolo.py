...
# LINKS GOOGLE DRIVE
# ==============================
CHECKLIST_DRIVE_URL = "https://drive.google.com/uc?export=download&id=1kda7U66av75JUJKPr71lN1kgXR5t0YbI"
LOGO_DRIVE_URL = "logo.png"
...
def calcular_progresso(df_cliente, checklist_dict):
    ...
    return int((itens_concluidos / total_itens) * 100) if total_itens else 0

def garantir_colunas(df):
    colunas = [
        "Cliente", "Etapa", "Status", "Motivo",
        "Responsavel_Etapa", "Responsavel_Cliente",
        "Participantes", "Data_Inicio", "Data_Conclusao", "Checklist"
    ]
    for col in colunas:
        if col not in df.columns:
            df[col] = ""
...
class PDF(FPDF):
    def header(self):
        try:
            response = requests.get(LOGO_DRIVE_URL)
            logo_bytes = BytesIO(response.content)
            self.image(logo_bytes, 10, 8, 30)
        except:
            pass
